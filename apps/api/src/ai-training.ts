          ...
          contents: [
            {
              ...
              parts: [
                {
                  text: `Seleccioná los IDs para una programación diaria. Respetá la intención, patrones, regiones, propósito y cantidad de cada bloque. Priorizá variedad y evitá repetir IDs usados en las últimas 72 horas cuando existan alternativas. Analizá también todas las actividades planificadas y realizadas: duración, intensidad, propósito y proximidad. Si hay varios partidos o estímulos similares, explicá qué regiones acumulan carga y sugerí una alternativa concreta de movilidad, recuperación u otra capacidad. Moderá volumen de piernas y potencia si hay deporte dentro de 24 horas, actividad física intensa reciente o readiness=tired. No inventes IDs. Para cada rutina escribí insight en español rioplatense explicando el propósito concreto y cualquier ajuste de recuperación; máximo 180 caracteres y sin diagnósticos ni afirmaciones médicas. Contexto: ${JSON.stringify({ experience: input.profile.experience, equipment: input.profile.equipment, jumpReady: input.profile.jumpReady, goals: input.profile.goals, goalNote: input.profile.goalNote, orientation: input.orientation, completedCount: input.completedCount, recentExerciseIds: input.recentSessions?.flatMap((session) => session.exerciseIds) ?? [], recentActivities: input.recentActivities ?? [], upcomingSportInDays: input.upcomingSportInDays, readiness: input.readiness })}. Opciones permitidas: ${JSON.stringify(dailySelectionCandidates(input))}`,
                },
              ],
            },
          ],
